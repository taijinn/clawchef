...
function runCommandStreaming(cmd, args, cwd) {
    return new Promise((resolve, reject) => {
        if (cmd === 'openclaw') {
            ...
            cmd = process.arch === 'arm64' ? '/opt/homebrew/bin/brew' : '/usr/local/bin/brew';
        } else if (cmd === 'git') {
            cmd = process.arch === 'arm64' ? '/opt/homebrew/bin/git' : '/usr/bin/git';
        } else if (cmd === 'npm') {
            cmd = process.arch === 'arm64' ? '/opt/homebrew/bin/npm' : '/usr/local/bin/npm';
        }
        ...
        child.on('close', (code) => {
            if (code === 0) resolve();
                ...
                // Extract just the meaningful error line from openclaw output if present
...
ipcMain.handle('approve-pairing', async (event, workspacePathInput, channel, code) => {
    ...
    try {
        ...
        let cmdArgs = ['pairing', 'approve', channel, code];
        sendLog(`> [EXEC] openclaw ${cmdArgs.join(' ')} (cwd: ${repoPath})`);
